...
class EnsembleSamplingAgent(BaseAgent):
    def __init__(self, num_models=10):
        ...
        # we're going to maintain SGDRegressor models
        # self.models = [linear_model.SGDRegressor(eta0=0.0001, learning_rate='adaptive') for _ in range(self.num_models)]
    ...
    # def update(self, observation, _, reward):
    ...
    #         # we also need to add some noise to the reward
    #         # noise = np.random.normal(0, self.sigma_w)
    #         noise = 0
    #         print(observation[DOSAGE_BUCKET_COLUMN])
    #         self.models[i] = self.models[i].partial_fit(new_features, [observation[DOSAGE_BUCKET_COLUMN]+noise])
    #     self.t += 1
